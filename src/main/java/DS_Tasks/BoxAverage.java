...
public class BoxAverage {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.print(boxAverage_Ivan(ans));
    }
...
// Return the avg of min and max (rounded up to the nearest int)
        result = (int) Math.ceil((double) (min + max) / 2);

        return result;
    }
    public static int boxAverage_Ivan(ArrayList<Integer> ans) {
        //this is default OUTPUT. You can change it.

        int result = 0;

        double max = Collections.max(ans);
        double min = Collections.min(ans);
